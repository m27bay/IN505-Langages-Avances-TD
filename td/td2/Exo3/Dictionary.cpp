...
/* Builders */
// Default
Dictionary::Dictionary() : size(0), first(nullptr) {}
...
Dictionary::Dictionary(const char* word, const char* def)
{
  this->size = 1;
...
/* Getters */
Definition* Dictionary::getFirst() const
{
  return this->first;
}
/* End getters */
...
/* Overloaded */
std::ostream &operator<<(std::ostream &flux, const Dictionary& other)
{
  if (!other.first)
  {
    flux << "{ *empty* }" << std::endl;
...
/* Methodes */
// Add
int Dictionary::addFirst(const char* newWord, const char* newDef)
{
...
int Dictionary::addEnd(const char* newWord, const char* newDef)
{
  Definition* newDefinition = new Definition(newWord, newDef);
  if (!newDefinition)
  {
...
int Dictionary::addIndex(const char* word, const char* def, int index)
{
...
int Dictionary::addAlphabetize(const char* word, const char* def)
{
